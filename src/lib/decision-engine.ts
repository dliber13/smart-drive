export type DecisionTier = "A" | "B" | "C" | "D" | "E"
...
function scoreIdentity(input: DecisionEngineInput): number {
  ...
  if ((input.identityStatus || "").toUpperCase() === "VERIFIED") {
    points += 7
  } else if ((input.identityStatus || "").toUpperCase() === "PENDING") {
    points += 3
  }
...
function scoreCashStructure(downPayment: number, tradeIn: number, vehiclePrice: number): number {
...
function scoreVehicleFit(vehiclePrice: number, maxVehicle: number): number {
  ...
  if (vehiclePrice <= maxVehicle * 1.10) return 4
  return 0
}
...
export function runDecisionEngine(input: DecisionEngineInput): DecisionEngineOutput {
  ...
  adjustedMaxPayment = clamp(adjustedMaxPayment, 0, monthlyIncome * 0.20)
