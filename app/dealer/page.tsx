...
export default function DealerPage() {
  ...
  if (!authChecked) {
    return <main className="min-h-screen bg-[#f7f4ee] p-8">Loading...</main>
  }
  ...
  return (
    <main className="min-h-screen bg-[#f7f4ee] text-[#111111]">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-8 py-8">
          <div>
            ...
            <p className="mt-3 text-[16px] text-black/60">
              {currentUser?.fullName} · {currentUser?.role} · {currentUser?.teamName || "No team"}
            </p>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-8 pb-16 pt-10">
        <div className="mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <h2 className="text-[64px] font-semibold leading-[0.92] tracking-[-0.065em] sm:text-[74px] xl:text-[88px]">
            ...
            <p className="mt-6 max-w-2xl text-[21px] leading-[1.55] tracking-[-0.02em] text-black/62">
              ...
              then submit files into your assigned team pipeline.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <Kpi label="Approved" value={totals.approved} />
            <Kpi label="Stips" value={totals.docs} />
          </div>
        </div>

        <div className="grid grid-cols-1 gap-8 xl:grid-cols-[1.02fr_0.98fr]">
          <div className="rounded-[36px] border border-black/10 bg-white p-8 shadow-[0_18px_45px_rgba(0,0,0,0.05)]">
            <div className="mb-8">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-4">
                <button
                  type="submit"
                  disabled={submitting}
                  className="rounded-[20px] bg-[#111111] px-8 py-4 text-[16px] font-semibold text-white disabled:opacity-60"
                >
                ...
                <button
                  ...
                  className="rounded-[20px] border border-black/10 bg-white px-8 py-4 text-[16px] font-semibold text-black/72"
                >
                  Clear Form
                </button>
              </div>
            ...
            {loadingDeals ? (
              ...
              <div className="space-y-4">
                {deals.map((deal) => (
                  <div
                    key={deal.id}
                    className="rounded-[28px] border border-black/8 bg-[#fcfbf8] p-6"
                  >
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      <div className="min-w-0">
                        ...
                        <div className="mt-4 grid grid-cols-1 gap-3 text-[15px] text-black/62 sm:grid-cols-2">
                          <div><span className="text-black/36">Stock #:</span> {deal.stockNumber || "—"}</div>
                          <div><span className="text-black/36">VIN:</span> {deal.vin || "—"}</div>
                          <div><span className="text-black/36">Vehicle:</span> {buildVehicleLabel(deal)}</div>
                          <div><span className="text-black/36">Income:</span> {formatCurrency(deal.monthlyIncome)}</div>
                          <div><span className="text-black/36">Credit:</span> {deal.creditScore ?? "—"}</div>
                          <div><span className="text-black/36">Created:</span> {formatDate(deal.createdAt)}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <Link
                          ...
                          className="rounded-[18px] bg-[#111111] px-5 py-3 text-[15px] font-semibold text-white"
                        >
                          Open UW
                        </Link>
                      </div>
...
function Kpi({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-[24px] border border-black/8 bg-white/74 p-5 shadow-[0_18px_42px_rgba(0,0,0,0.05)]">
      <div className="text-[11px] uppercase tracking-[0.28em] text-black/36">{label}</div>
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="w-full rounded-[20px] border border-black/10 bg-white/88 px-5 py-4 text-[16px] text-[#111111] outline-none placeholder:text-black/28"
      />
    </label>
  );
}
